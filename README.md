...
roda em segundos a partir do CSV já extraído — é por isso que
...
`tpcds-postgres:sf{1,2,4}`) — essas imagens já têm os dados do TPC-H/TPC-DS
...
pipeline de ML nunca os lê — eram os maiores responsáveis por inchar os
...
por design — variância zero, deliberadamente fora do vetor X
...
tasks — 668 `done` + 4 `abandoned`), cobrindo os 3 tiers × 7 combinações de
...
### Importância de features — SHAP (modelo M1)
...
`shared_buffers` (8,3%) — contraria a intuição comum de que memória sempre
...
- **Outliers mantidos no dataset.** 7 tasks da Rodada 2 têm
  ...
  13.324ms para 636ms, mas o ρ mudaria de 0,966 para apenas 0,969 — ganho
